  ...
  return (
    <div className="relative h-full w-full flex flex-col overflow-hidden">
      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-6 py-6 md:px-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
            {reasons.map((r, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: i * 0.07 }} className="bg-[#111] border border-white/5 p-4 rounded-sm relative group overflow-hidden">
                ...
                <div className="text-white/25 group-hover:text-primary transition-colors duration-300 mb-3">{r.icon}</div>
          ...
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <p className="text-[10px] font-mono text-primary uppercase tracking-[0.35em]">Highlights / Achievements</p>
              <span className="h-px flex-1 bg-white/10" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {highlights.map((item, i) => (
                <motion.div key={item.title} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, delay: 0.15 + i * 0.1 }} className="border border-primary/20 bg-[#101010] p-5 rounded-sm relative overflow-hidden">
                  <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(179,18,23,0.14)_0%,transparent_70%)]" />
                  <div className="relative z-10 flex items-start gap-3">
                    <div className="text-primary mt-0.5">{item.icon}</div>
                    <div>
                      <h3 className="text-white uppercase tracking-wide text-sm font-bold mb-1">{item.title}</h3>
      ...
      <div className="relative z-10 border-t border-white/5 bg-[#0b0b0b]/90 px-6 py-4 flex items-center justify-between">
        <button data-testid="why-back-btn" onClick={onBack} className="flex items-center gap-2 text-white/40 hover:text-white text-xs font-mono uppercase tracking-widest transition-colors">
          <ChevronLeft className="w-4 h-4" />
          Back
        </button>
        <span className="text-[10px] font-mono text-white/20 uppercase tracking-widest">Step 2 of 6</span>
        <motion.button data-testid="why-next-btn" onClick={onNext} whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }} className="flex items-center gap-2 px-6 py-2.5 border border-primary text-white font-bold text-xs uppercase tracking-widest hover:bg-primary transition-colors duration-300">
          I'm In, Next
