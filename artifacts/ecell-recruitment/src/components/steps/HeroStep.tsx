...
export default function HeroStep({ onNext }: HeroStepProps) {
  return (
    <div className="relative h-full w-full flex flex-col items-center justify-center overflow-hidden">
      ...
      <motion.div
        className="absolute left-0 top-1/2 -translate-y-1/2 pointer-events-none select-none -translate-x-[26%] sm:-translate-x-[12%]"
      ...
      >
        <img
          src={daliMask}
          alt=""
          className="w-[34vw] max-w-[520px] min-w-[130px] sm:w-[74vw] md:w-[62vw] lg:w-[52vw] object-contain"
          style={{ opacity: 0.18, filter: "grayscale(30%) brightness(0.72)" }}
        />
      </motion.div>

      <motion.div
        className="absolute right-0 top-1/2 -translate-y-1/2 pointer-events-none select-none translate-x-[26%] sm:translate-x-[12%]"
        initial={{ opacity: 0, x: 40 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 1.2, delay: 0.4, ease: "easeOut" }}
      >
